...
“新版结账上线后退款变多了，要不要回滚？”这项调查从模糊需求开始，材料包含业务定义、订单、客服反馈和发布记录；第二轮还会改变指标、补充事件数据并调整交付目标。Agent 要选择调查步骤、追问缺口并更新判断，程序负责可重复计算。[第七章拆解](book/07-general-agent.md#changing-task)与[分阶段模型实验](examples/investigation/README.md)提供材料、操作和参考答案。

这类任务体现的是面对变化时组织工作的能力，不能仅凭案例宣称 Agent 胜过固定程序。下面的小型订单修复保留为工具循环入门练习，不承担证明 LLM 必要性的任务。
