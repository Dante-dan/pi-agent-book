...
放回退款调查，扩展适合负责获取带版本的指标说明、提供查询接口、保存尚未结束的查询 ID。至于下一步查客服还是发布记录，可以由模型根据观察选择；负责人是否批准暂停灰度，则是另一项决定。不要把“发现退款率上涨就回滚”的业务结论直接写入通用 runtime。第七章的[变化任务](07-general-agent.md#changing-task)会同时检验接口是否稳定、任务计划是否能够改变。
