...
你发现订单程序算出的金额不对，想请 AI 帮忙。把问题发到聊天框，它可能给你一段修改建议；换成 Pi，你可以让它读取电脑上的程序、修改文件、运行检查，再交回结果。要使用这种助手，我们先要弄清它怎样从“给建议”走到“实际做事”。
...
让我们想象一下，你正在参与开发一套电商订单系统。用户选好商品后，会进入结账页，确认地址、运费和总价，然后付款。团队刚上线了一版新的结账页，调整了运费的展示位置和支付按钮的文字。为了先观察使用情况，目前只有一部分用户会进入新版页面。

这时，负责人找到你：

> “新版结账上线后，退款好像变多了。你帮我查一下，是不是这次改版出了问题？明早我们要决定，要不要恢复旧版。”

你手上有几份订单导出、客服记录和发布说明。订单里能看到谁申请了退款，客服记录里有用户对运费的抱怨，发布说明则写着这次改了哪些地方。但这些资料还没有整理在一起，你也不知道退款增加是否真的与改版有关。

如果想请大语言模型（LLM）帮忙调查，我们需要先考虑，它要怎样接触这些资料，又怎样根据资料推进工作。

首先，要让它看到实际材料。只把负责人的话发给模型，它最多只能列出一些可能原因。要查这次发生了什么，它需要读取订单、查看客服原话，并找到对应的发布记录。数据比较多时，还需要运行程序计算，而不是在回复里凭印象估算。

其次，要让它分清哪些问题已经有答案，哪些还需要确认。负责人说的“退款”，是用户提交了申请，还是钱已经退回？用户抱怨运费，能不能说明新版页面让人误解？这些问题会影响后面的计算和判断。材料不足时，我们希望它指出缺口、提出有针对性的问题，而不是急着认定新版有问题。

调查也可能做到一半才发现，负责人真正想决定的是“要不要继续把新版开放给更多用户”。这与“把已经使用新版的用户全部切回旧版”是不同的选择。模型需要根据补充说明调整计划，保留已经确认的事实，再去查与新问题有关的证据。

最后，调查需要留下可以检查的结果：数字怎么算出来，结论依据哪条记录，还有什么尚未确认。模型可以帮助整理这些证据并提出建议，负责人再据此作出上线决定。

接下来，我们会用 Pi 逐步搭起完成这类工作的基础：让模型通过工具读取资料、运行程序，在多轮对话中保存进展，并在需求变化后继续工作。第一章先用一个小型订单修复熟悉这些操作；等读懂运行、上下文和记忆机制后，第七章再一起完成[这次调查](07-general-agent.md#changing-task)。你也可以在[配套实验](../examples/investigation/README.md)中取得资料，亲自观察 Pi 会怎样处理。

## 1.1 先认清我们在使用什么

先看刚才的调查请求怎样开始。你把负责人的话输入 Pi，接下来至少要发生三件事：理解要查什么，读取实际资料，再根据结果决定下一步。这些事情分别由模型、工具和组织它们的程序承担。先认清各自的职责，后面看运行记录时才知道是谁做了什么。
...
调查所需的资料还放在你电脑里：订单在 CSV 文件中，客服反馈在另一份文档中。即使 Pi 已经在这个目录启动，模型也不会自动知道每个文件的内容。它要先提出读取请求，工具执行后，读到的片段才会成为它能使用的证据。

这些文件，以及保存它们的系统，都属于 Agent 所处的**环境**。环境还可以包括数据库、网页、用户、其他 Agent，甚至机器人的物理世界。Agent 通过观察接口取得信息，通过行动接口改变环境。
...
现在准备在自己的电脑上试一次。你需要能启动 Pi，也需要让 Pi 连上一个可用的模型服务。安装成功只解决了前一件事，接下来会依次检查运行环境、安装程序和配置模型。
...
完整的退款调查涉及多份资料和多轮判断。第一次操作时，我们先取一个容易看清结果的环节：订单汇总程序把已退款订单也计入了收入，需要修复。练习目录已经备好程序、三条虚构订单和检查脚本，你不需要先读懂源码。先运行检查，看到它错在哪里，再让 Pi 尝试修复。
...
修复完成后，你可能想关掉终端，明天再检查另一份数据；也可能需要在几个旧任务之间切换。这时不能只记得“上次聊到哪里”，还要知道怎样找回对应会话。下面按这些实际操作列出入口；表中的斜杠命令在 Pi 输入框执行，`pi -c` 等启动命令在终端执行。
...
做第二次修复时，你又要提醒 Pi：“金额用整数分，别改原始订单，修改后运行检查。”这些约定每次都相同，可以放进项目文件，让 Pi 启动时读取。先在练习目录写一份 `AGENTS.md`：
