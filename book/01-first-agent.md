...
### 全书的主线：一个开始时没有完整定义的问题

假设负责人说：“新版结账上线后，退款好像变多了，明早要不要回滚？”他没有告诉你退款指申请还是完成，也没有给出分析步骤。资料散在订单导出、客服反馈和发布记录里；调查到一半，他又补充：“我真正担心的是运费展示让人误解，先决定是否暂停扩大灰度。”新数据还揭示了前后两批订单的观察时间不同。

这个任务不能靠事先写好的一个加总公式交付。需要先识别术语里的歧义，从材料选择调查线索，判断什么能够计算、什么必须追问，再根据新证据修改计划。LLM 可以承担这些解释和选择；计算、文件关联、证据保存仍交给工具和程序。最终业务决定保留给负责人，模型不能用一段流畅解释代替证据。

这将是全书的综合主线，第七章会提供[完整推演](07-general-agent.md#changing-task)，配套目录提供[分阶段实验与答案](../examples/investigation/README.md)。下面先用很小的订单修复认识工具循环。**这个小练习完全可以手写程序完成，它只用于看清运行机制，不用于证明任务需要 LLM。** 区分“小练习验证机制”和“综合任务验证适应能力”，才不会把调用了模型当成使用 Agent 的理由。
