...
本章不要求你写过 Agent，也不要求你懂 TypeScript。你只需要愿意打开终端，在一个练习目录里观察文件如何变化。我们先完成一件小事，再解释它背后的机器是怎样工作的。
...
参考判断：第一题要指向真实代码、数据或失败输出，而不是模型的猜测；第二题分别对应编辑/写入和执行检查；第三题没有完成修复；第四题因为外部文件可能在会话之外改变，会话记录不代表当前环境快照。
