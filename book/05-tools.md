...
你正在维护订单系统中一个负责查询状态的服务。团队已经决定把网络失败后的最多重试次数从三次改成五次，让 Pi 帮忙修改。它需要先找到配置，读清当前内容，只改目标字段，再检查文件结果。

如果工具只返回一句“成功”，你还不知道它是找到了文件、写入了新值，还是已经验证服务按新配置运行。同样，搜索只返回前几条结果却没有说明，也可能让模型误以为已经查遍项目。本章要讨论的工具设计，就从这些具体的信息缺口开始。
...
退款调查里也有相同需要：查询返回 `refund_rate: 0.4`，模型还得知道它按申请还是完成计算、对应哪批订单。后面讨论返回格式和来源时，可以用这个问题检查：这些信息是否足够支持下一步判断？

## 5.1 工具是一份契约，不只是一个函数

搜索提示配置位于一个文件的第 230 行附近。模型想读取周围 80 行，就得知道工具是否接受起始行、从 0 还是从 1 计数、读不到时会返回什么。若这些约定不明确，即使底层读取函数能运行，调用方也可能拿错内容。

因此，工具可以理解为一个带说明书的函数。说明书交代用途和输入，函数执行操作，返回值说明结果；失败、截断、取消和副作用也属于这份契约。
...
刚才的重试配置任务需要搜索、读取、修改和检查。换成退款调查，可能又需要运行分析程序、整理资料。Pi 不必为“修改重试次数”和“调查退款”分别写一套固定流程，而是让模型组合基础操作。先看它默认提供什么，再按这些操作对环境的影响分类。
...
你也许已经用过 `cat` 查看文件，用 shell 命令搜索文本，再运行脚本修改配置。这些工作看起来都能通过 `bash` 完成。那么，让模型只学习一个 shell 入口，是否就足够了？

实际修改时，模型还要同时处理引号、转义、定位范围和错误输出。`read/write/edit` 将其中一些经常重复的细节收进工具接口，让一次调用的含义更明确。

<a id="argument-fidelity"></a>

### 参数经过的语言越多，失真机会越大

团队要把一段带价格和代码示例的说明写入文件。要求原样保留下面这句话，美元符号和反引号都是正文：
...
模型找到两份配置，一份属于演示服务，另一份属于线上服务。即使它给出的 `path` 确实是字符串、`edits` 确实是数组，也仍可能选错文件。这里需要分清两种检查：输入形状是否合规，以及这次操作是否符合任务。
...
同一个配置文件里，演示服务和另一个服务都写着 `retries: 3`。如果模型只请求替换这段文字，工具不能猜它指哪一处。拒绝这次含糊修改，并让模型读取更多上下文，比悄悄改掉两处更容易恢复。

专用工具可以围绕自己的操作给出这种明确反馈：`edit` 拒绝歧义匹配，`read` 标出返回片段的位置，`write` 说明创建或覆盖结果。shell 的输出则由具体程序决定。

**【内建】** 这个版本的 `edit` 接受 `edits` 数组，每一项针对原始文件中的一个不重叠区域。它不是先执行第一项，再让第二项搜索第一项修改后的内容。工具会保留文件开头可能存在的编码标记 BOM 和换行风格，并生成 diff（修改前后的差异）与标准补丁。匹配实现包含有限的文本归一化与模糊回退，不能宣称它是逐字节替换；调用时仍应提供实际读到的唯一旧文本，不能依赖模糊匹配猜中目标。[edit 主流程](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/coding-agent/src/core/tools/edit.ts#L148)；[匹配与重叠检查](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/coding-agent/src/core/tools/edit-diff.ts#L295)
...
Pi 还不知道重试次数放在哪里。搜索 `retries` 后，命中可能同时来自真实配置、测试样例和说明文档。它需要先浏览位置与片段，找到最相关的候选，再读完整配置块，确认服务名称和上下文。

搜索回答“哪里值得看”，读取回答“那里具体写了什么”。把所有命中的全文一次交回，反而会让定位阶段携带大量暂时无关的材料。
...
搜索已经把 Pi 带到一个几千行的配置文件，目标在第 230 行附近。它没有必要把整份文件都读一遍；但若任务改成检查所有重试配置，就必须知道当前只读到了哪一段、还有多少没读。起始位置、读取数量和截断通知正是为这两种需要服务的。
...
Pi 想读取服务配置、部署说明和测试要求。这三份稳定文件可以同时读；同一份说明刚读过，也可能值得复用结果。但如果你在旁边的编辑器里改了配置，复用旧内容就会出错。

读取通常不改变业务数据，所以相互独立的读取容易并发，也有缓存价值；结果能否复用，还取决于资源版本和权限是否变化。“只读”本身不能保证下一次仍是同一个答案。
...
启动工具返回 `jobId` 后，界面可以显示“已登记任务”，模型也有了以后查询的凭据。但这时构建还没有完成，不能把这个 ID 当作验收结果。成功、失败和已取消必须是清楚的终态；排队、运行中和取消请求已接收都不是成功。百分比没有可靠来源时，可以返回“正在安装依赖”这样的阶段信息，不要编造进度。超时重试 `start` 还可能重复创建任务，所以示例中的 `requestId` 应由服务端按约定去重；查询和取消都应检查调用者是否有权访问该任务。
...
退款调查中，有用户说结账页看不清运费。设计同事给你一张截图，希望 Pi 检查运费提示是否被按钮遮挡。若只把截图里的字提取出来，金额还在，按钮与文字的位置关系却可能丢了。

这类布局问题需要保留图像证据。另一项任务如果只是从几页纯文字扫描件里找条款，可靠的文字提取可能更方便搜索与引用。工具返回什么形态，要从要判断的问题出发。
...
团队把订单查询、客服搜索、发布记录和库存系统都接入助手，每个系统又有几十个接口。当前只需调查结账问题，却把几百个工具定义全发给模型，它还要从许多相似名称里辨认用途。
...
负责人这次只让 Pi 审阅配置，不允许修改。你把“不要写文件”放进提示后，还希望运行环境确实限制写入。先从模型能调用的工具集合收紧，再区分哪些限制还需要操作系统或服务端保证。
...
Pi 尝试修改配置，工具返回“失败”。它不知道是路径写错、旧文本不唯一，还是服务端已经修改但回执丢了；此时原样重试可能无效，也可能重复执行。失败结果必须给出足够线索，让下一轮能选择合适的恢复方式。
...
回到章首的配置修改：把演示服务的重试次数从三次改成五次，其他字段保持原样。下面用一份本地文件观察搜索、读取、修改和复查怎样衔接，再故意制造截断与歧义匹配，看看工具怎样帮助模型调整动作。
