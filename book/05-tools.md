...
退款调查还会改变工具的使用方式。第一轮按订单行统计申请，第二轮要根据新 schema 将退款事件关联回订单；同一订单的申请与完成不能算成两笔退款。工具应保留订单 ID、事件类型、来源位置和截断信息，让模型能够发现这种变化。若只返回一个没有定义的 `refund_rate: 0.4`，再聪明的模型也难以判断它究竟代表什么。工具契约因此需要支撑未知问题的调查，不只服务于一个预定公式。
