...
你开始每周用 Pi 整理门店报告。第一次，它遗漏了退款订单检查；你指出问题，它修复脚本并重新运行。第二天换一个目录开始新会话，同样的遗漏又出现了。你想保留下来的，不只是昨天修好的报告，还有下次遇到类似任务时能用上的做法。

本章先分清一次对话纠正与长期能力变化，再学习从运行记录定位失败，把修复放到知识、Skill 或程序中；之后准备参与调试和独立验收的不同样例，最后用一份改进报告决定发布还是回退。我们继续沿用第七章的月报练习，并检查同样的方法怎样用于退款调查。

昨天的纠正可能只进入了那次对话的上下文。要让下次任务也用上它，就需要保存某种持久改变，并让后续运行找到、理解和使用它。下面围绕这条路径检查改进如何生效。
...
发现重复错误后，你可以再提醒模型一次，把检查步骤写进 Skill，或者直接修改报告程序。这三种做法都可能改善下一次结果，但影响的范围不同：有的只在当前对话有效，有的能被后续任务读取，有的由程序每次执行。谈“Agent 学会了”之前，先说明改变发生在哪里。
...
决定了修改对象，验证方法也要随之改变。修改知识，需要证明来源准确且适用；修改 Skill，需要证明模型会在合适任务中加载并遵循；修改程序，需要检查输入输出与异常；换模型，则要重新评估整个组合的行为。只说“Agent 学会了”会遮住真正的工程问题。

## 8.2 轨迹是调查材料，评分不是最终回复

Pi 最后说“报告已经完成”，你打开文件却发现退款被重复计入。仅凭最终回复，无法判断它是没读规则、脚本算错，还是已经看到错误却照样宣布成功。要修正这个问题，得回到它实际读过什么、执行过什么、得到了什么结果。

这些过程记录称为**运行轨迹**，包括用户输入、模型消息、工具参数、工具结果以及相关事件。它们让你能沿任务过程找到第一次偏离要求的位置。
...
这次排查已经结束。你确认了本月指标定义，也找到了漏做重复检查的原因，还修好了一段代码。如果把这些内容全部写成“下次注意退款”，下次任务仍不知道要读哪条规则、运行哪项检查。应按经验的用途分别保存。
...
报告还没交付时，Pi 发现自己的核对 Skill 少了一步。它可以先保存这个问题，但若立刻改掉团队共享的 Skill，其他正在运行的任务就可能使用未经检查的新流程。

可以把两件事分开：当前任务先按已知要求完成并保存证据；任务结束后，另一个改进流程读取轨迹、提出修改、运行评估，再决定是否发布。这样既能积累经验，也能在影响后续工作前检查修改。
...
候选 Skill 或脚本已经通过了约定检查，团队准备下周开始使用。如果所有人都直接覆盖旧文件，出现新问题时就难以确认谁用了哪版、该退回哪里。发布需要记录版本与检查依据，也需要保留旧版入口。

Pi 可以帮助编写 Skill 和扩展，但写出修改与发布修改是两个步骤。先形成候选版本，经过检查，再把它接入真实任务。
...
团队更换了指标查询接口，新的 Skill 已经写好，旧 Skill 却还留在能力目录里。两份说明都声称适用，模型可能读到旧接口，或者在冲突中选择错误。继续追加说明不能消除这个问题，还需要维护哪些规则仍然有效。
...
假设团队准备把本周的报告助手用于下周，你负责检查一次改进是否值得发布。沿用上一章的月报任务，先记录未修改版本在正常输入、重复订单、未知状态和一单多行数据上的行为。随后只选一个失败类别修改 Skill 或程序，并写清预计改变哪个可观察行为。
