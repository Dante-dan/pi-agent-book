...
Pi 直接适合承载前四类外围改变：读取知识、加载过程、运行程序、通过扩展调整运行方式。它调用模型进行推理，不会因为写入了一个 Skill 就自动更新模型权重。更新 Pi 软件、安装新包、替换提示词，也都不等于进行了参数训练。
...
```text
若必需事实未进入上下文：修检索或加载路径
若事实已出现但步骤遗漏：修 Skill 或任务编排
若参数合法但业务不合法：修工具或服务端校验
若工具异常后被误报成功：修错误传播与验收门槛
若只是外部服务故障：修恢复策略，保留故障证据
若换一种任务仍反复无法理解：再考虑模型选择或更大改造
```
