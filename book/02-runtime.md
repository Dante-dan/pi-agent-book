...
退款调查第一轮使用 METRIC-01 的申请率；第二轮，负责人通过 DEC-02 明确本次要看完成率。应用需要保留第一轮报告和当时采用的定义，同时把“本轮适用规则”的临时注入替换为新决定。下面的 `custom` 消息形状参考 Pi 的自定义消息；业务检索函数与筛选策略是示意，不是 Pi 自动实现的知识库。
