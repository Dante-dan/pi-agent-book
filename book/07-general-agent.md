...
运营同事拿到几百份门店报表，希望找出本月退款变化的原因。他不打算开发一款软件，但需要读取不同文件、统一字段、计算比例，再把数字和客服反馈放在一起解释。只靠聊天回复，很难检查这些数据处理是否真的发生过。

Pi 能读写文件、执行程序，我们可以让它临时编写处理脚本，逐步完成这项工作。本章先解释代码为什么能充当临时工具，再回到第一章的退款调查，跟随两轮证据调整计划；随后把同样的工作方式用于团队文档检索，最后讨论执行权限与多 Agent 分工。可手算的 CSV 会保留为计算练习，帮助你核验其中一个具体环节。
...
调查做到一半，目录里已有原始导出、清洗后的表、临时脚本和两版报告。第二天继续时，如果不知道哪份是原数据、哪份只是中间结果，就很难重算并解释差异。

可以用目录把这些用途分开，让 Pi 继续任务时有地方查找。这是本书建议的任务布局，需要由应用或使用者建立：
...
现在回到第一章的订单系统。团队把新版结账页开放给了一部分用户，改动包括运费的展示位置和支付按钮的文字。负责人看到退款似乎增加了，希望你查明原因，为第二天是否恢复旧版提供依据。

这次我们准备实际推进调查。手头有订单导出、客服摘录和发布记录，但没有一份资料直接回答“是不是改版造成的”。如果让 Pi 来做，就需要先让它检查这些文件，弄清能计算什么、还缺少什么，再决定下一步。

例如，订单中的退款字段可能记录申请，也可能记录完成；新版只对部分用户开放，“上线后的订单”不一定都来自新版。客服提到运费问题，值得进一步查，但也可能有其他用户因为物流延迟退款。如果不先分清这些情况，一张计算正确的汇总表也可能被用来支持错误的决定。

我们希望 Pi 能从材料中发现这些问题，选择合适的分析方法，并在必要时向负责人确认。已经明确的统计交给程序反复执行；需要查看哪份资料、哪些判断还站不住脚，则随着调查进展继续调整。下面按两轮资料交付，观察这个过程。
...
得到这两个比例后，计算环节已经完成，但负责人提出的问题还没有回答。还需要判断：这两个数是否在比较同一种东西，它们支撑哪一级结论，下一项值得取得的证据是什么。
...
调查中需要按门店比较数字。为了先核对程序有没有正确计算，我们取一份可以手算的四行数据，明确统计规则，再比较输出。这个环节做好以后，才能把数字带回前面的证据判断。

新建练习目录，在 `input/orders.csv` 放入以下虚构数据。`amount` 表示原订单金额，单位为元；退款订单用 `status=refunded` 标记。
...
一位刚加入订单团队的同事在代码里发现，账单每天集中处理一次，而旧文档写的是实时处理。他问：“我们为什么把账单处理从实时改成批量？”会议纪要和变更记录可能保存着当时的原因，但他还不知道该找哪份。

模型可以解释两种处理方式的一般差别，要回答这个团队为什么改，却必须先找到项目证据。
...
调查脚本可以在工作目录里反复修改，计算错了就修正后重跑。如果接下来的任务变成“为一笔真实订单预留库存”，错误执行却会影响其他订单。两者都能用代码表达，但允许模型自由改写到什么程度，不能采用同一种安排。

文件批处理、数据清洗、格式转换、生成图表和维护文档，常需要临时程序处理多种输入。涉及库存等严格业务约束时，则应优先让专用服务负责关键操作。

具体到库存，一个业务服务可以提供 `query_stock` 和 `reserve_stock`，在服务端检查仓库范围、库存版本，以及同一请求是否已执行过。后一点叫作幂等性：重复提交同一业务请求，不应重复预留库存。如果让模型自由编写 SQL 修改生产表，虽然灵活，却把本可由服务保证的业务约束转移给了临时程序。通用能力应帮助编排业务接口，而不是取消业务边界。
...
Pi 已经把月报写入 `output/`，你准备核对口径和排版。文件生成是一个明确进度，但还没有发给团队。如果工具把生成和发送绑在一起，每次修订都可能把半成品再发一遍。

将生成与发布分开后，可以先验收本次产物，再通过发布系统交付。若用户已明确授权符合约定的报告自动发布，应用可按该授权执行，同时核对产物是否满足条件。
...
明早要交调查结果，待查的材料分成订单数据、客服反馈和发布记录三组。你可能想让三个 Agent 分别处理，最后由主 Agent 汇总。问题随之而来：它们是否采用同一指标定义，是否会覆盖同一个文件，其中一个失败后谁负责收尾？

多 Agent 协作需要先约定各自的目标、输入、产物、共享资源和失败处理，才能判断这项分工是否真正可并行。
