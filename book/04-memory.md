...
在综合调查中，记忆需要保留的不只是数字。第一轮采用的是申请率，第二轮负责人改为完成率；这两个数各自有定义、来源和适用时间，不能把旧答案当作错误直接覆盖。若想分别探索“运费展示”和“物流延迟”两个假设，可从已经确认的共同事实处分叉，并在每个分支保留支持与反对证据。树形历史保存的是调查路线，不会让某个假设自动成为事实，也不会替你保存或恢复对应的磁盘快照。下面先把这些状态分清，再演示实际操作。
