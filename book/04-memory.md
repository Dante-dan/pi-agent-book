...
昨天下班前，你和 Pi 已经查到报表误差的位置，但还没修改程序。今天重新打开终端，你想接着做，并希望它仍知道项目的金额约定、以及你习惯先看结论的表达方式。这些信息都曾经出现过，却未必通过同一种方式保存，也未必会在这次启动后自动可用。
...
退款调查也会遇到同样的问题：负责人改变退款率定义后，第二天仍可能追问“昨天那个数是怎么算的”。因此既要保留旧定义的来源，也要说明本次采用的新定义。下面从保存范围开始，再演示恢复旧会话与选择调查分支。

## 4.1 先分清四种保存范围

“继续昨天的修复”和“以后所有报告先写结论”，对保存信息的要求不同。前者要找回特定任务的进展，后者可能需要在许多新任务中生效。即使都已经写到了磁盘，也还需要在恰当的时候带回模型输入。可以先按下面四种范围选择保存与读取方式。
...
先解决最直接的问题：昨天的排查还没结束，今天怎样接着做？你需要找回那次会话，而不是重新输入全部经过。Pi 将工作记录保存在本地会话文件中，启动命令可以选择恢复哪一份。
...
你正在帮同事排查一张金额对不上的报表，只讨论原因，暂时不改文件。Pi 已确认输入单位都是元；你怀疑是显示的小数位造成差异，让它先研究方案 A：调整导出格式。几轮后，这条路没有解释误差，你想回到确认单位的位置，另试方案 B：逐笔检查累加过程。

如果前面的讨论仍有用，直接说“现在检查聚合算法”就够了；如果想保留旧尝试，同时从共同事实重新比较另一条路线，就适合分叉。下面用这次排查说明如何选择续接位置。

这种操作也适合改写早先含糊的提问。旧尝试仍会保存，下一次模型请求则沿你选中的路线准备上下文。
...
刚才是在同一次排查中换方向，`/tree` 已经够用。若想把一次旧提问单独拿出来做实验，或者从当前进度另开一个任务，就需要新会话文件。下面按这几种目的区分命令。
...
昨天排查日志太长，你执行过 `/compact`，然后退出了 Pi。今天恢复时，如果把旧日志全部重新塞回去，压缩就白做了；只读摘要，又可能漏掉摘要之后刚完成的检查。恢复程序因此要知道：哪部分由摘要代表，哪些旧消息保留原文，之后又新增了什么。
...
假设你写了一个扩展，每次处理会话后记下“已经索引到条目 c”，下次就不用重做前面的索引。程序需要这个进度，模型却通常不需要读它。另一条信息“用户已确认金额单位为元”则必须让模型看到，才能影响后续计算。

这说明“写进会话文件”还不足以决定用途。Pi 区分两类条目：
...
几次排查之后，团队把确认过的指标定义、变更原因和检查方法记在文档里。新同事问“为什么这份报表不计入退款申请”，你希望 Pi 找到当时的决定，而不只是给一个通用解释。

如果资料只有十条，先用可读文件就很好。可以在全局约定中记录确认过的表达偏好，在项目里用 `docs/decisions.md` 保存设计决定；需要时让 Pi 读取。后者只是我们选择的文件组织方式，Pi 不会因为文件叫这个名字就自动加载它。

等到几个项目积累了几百次决定，逐份阅读就会慢下来。同名报告、旧口径和临时例外也会混在一起。这时要先决定怎样保留对象、来源和有效时间，再选择存储与检索方式：
...
新同事可能问“报表合计有偏差”，旧记录却写“金额累计误差”。只按原句匹配容易漏掉相关材料；向量检索可以用来寻找这类不同措辞但语义相近的片段。具体做法是把文本映射成数值表示，再用距离检索相近内容。但语义接近不等于事实正确，更不意味着记录属于当前用户或适用于当前版本。
...
找到一整份长文档还不够。模型可能只需要“金额单位”所在的小节，而搜索结果必须能回到原文核对。为此，要先把资料整理成可检索的片段并建立索引：先提取文档文字，按章节或完整语义段分块，为每块保存文档 ID、标题路径、版本和原文位置；再建立关键词索引或向量索引。查询时先限定权限和版本，再搜索、合并候选，必要时重新排序，最后读取原文。对表格不要把表头和行拆得失去关联，对代码不要把关键定义切成无法理解的碎片。
...
你让 Pi 临时生成一份给海外同事看的美元报表，下个月又要生成正式人民币报表。如果记忆系统把这次要求概括成“项目默认使用美元”，下次检索得越准确，反而越容易稳定地拿到错误规则。保存之前，必须分清一次例外和长期约定。

要保留这类信息，记录至少应说清楚对象、事实、适用范围、来源和时间。正式报表的长期约定可以这样保存，临时例外则应有自己限定的任务范围。下面的结构是应用层示例，不是 Pi 规定的 schema：
...
假设明天还要继续今天的报表核对，你需要知道：恢复原会话能带回什么，新会话又缺少什么。下面用代号和编号作可追踪的标记，依次比较会话恢复、分支和显式读文件。
